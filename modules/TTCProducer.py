    ...
    def analyze(self, event):
        ...
        for imu in range(0, event.nMuon):
            ...
            if abs(muons[imu].eta) > 2.4 or muons[imu].sip3d > 8 or abs(muons[imu].dxy) > 0.05 or abs(muons[imu].dz) > 0.1 or muons[imu].miniPFRelIso_all > 0.4:
                continue
            if not (muons[imu].looseId and event.Muon_corrected_pt[imu] > 10):
                continue
        ...
        for iele in range(0, event.nElectron):
            ...
            if abs(electrons[iele].eta) > 2.5 or electrons[iele].sip3d > 8 or abs(electrons[iele].dxy) > 0.05 or abs(electrons[iele].dz) > 0.1 or electrons[iele].miniPFRelIso_all > 0.4:
                continue
            if not (electrons[iele].mvaFall17V2noIso_WPL and electrons[iele].lostHits < 2 and electrons[iele].pt > 10 and electrons[iele].convVeto):
                continue
            if ((abs(electrons[iele].deltaEtaSC+electrons[iele].eta) < 1.479 and electrons[iele].sieie < 0.011) or (abs(electrons[iele].deltaEtaSC+electrons[iele].eta) > 1.479 and electrons[iele].sieie < 0.03)) and electrons[iele].hoe < 0.1 and electrons[iele].eInvMinusPInv > -0.04:
                if electrons[iele].mvaTTH > 0.25 and electrons[iele].tightCharge == 2:
                    if (electrons[iele].pt > 20):
                        electron_v4_temp.SetPtEtaPhiM(
                            electrons[iele].pt, electrons[iele].eta, electrons[iele].phi, electrons[iele].mass)
                        electron_v4_temp_raw.SetPtEtaPhiM(
                            electrons[iele].pt/electrons[iele].eCorr, electrons[iele].eta, electrons[iele].phi, electrons[iele].mass/electrons[iele].eCorr)
        ...
        for itau in range(0, event.nTau):
            ...
            if taus[itau].pt > 20 and abs(taus[itau].eta) < 2.3 and taus[itau].idDecayModeOldDMs and taus[itau].idDeepTau2017v2p1VSe >= 4 and taus[itau].idDeepTau2017v2p1VSjet >= 4 and taus[itau].idDeepTau2017v2p1VSmu >= 1:
                ...
                for ilep in range(0, len(fakeableLeptons)):
                    if tau_v4_temp.DeltaR(fakeableLeptons[ilep]) < 0.4:
                        pass_tau_lep_Dr = 0
                if pass_tau_lep_Dr:
                    nHad_tau = nHad_tau+1
        ...
        for ijet in range(0, event.nJet):
            ...
            for ilep in range(0, len(tightLeptons)):
                if jet_v4_temp.DeltaR(tightLeptons[ilep]) < 0.4:
                    pass_jet_lep_Dr = 0
            for ilep in range(0, len(fakeableLeptons)):
                if jet_v4_temp.DeltaR(fakeableLeptons[ilep]) < 0.4:
                    pass_jet_lep_Dr = 0
        ...
        for ijet in range(0, event.nJet):
            if not (ijet in tightJets_id_in24):
                continue
            if (ijet in tightJets_b_DeepCSVmedium_id):
                continue
            # loose/mudium WP
        ...
        tightJets_b_DeepCSVmedium_id.extend(
            np.zeros(event.nJet-len(tightJets_b_DeepCSVmedium_id), int)-1)
        tightJets_c_DeepCSVmedium_id.extend(
            np.zeros(event.nJet-len(tightJets_c_DeepCSVmedium_id), int)-1)
        tightJets_c_DeepCSVloose_id.extend(
            np.zeros(event.nJet-len(tightJets_c_DeepCSVloose_id), int)-1)

        self.out.fillBranch("tightJets_id_in24", tightJets_id_in24)
        self.out.fillBranch("tightJets_id_in47", tightJets_id_in47)
        self.out.fillBranch("tightJets_b_DeepCSVmedium_id",
                            tightJets_b_DeepCSVmedium_id)
        self.out.fillBranch("tightJets_c_DeepCSVmedium_id",
                            tightJets_c_DeepCSVmedium_id)
        self.out.fillBranch("tightJets_c_DeepCSVloose_id",
                            tightJets_c_DeepCSVloose_id)
        ...
        if ttc_nl:
            ...
            if len(tightLeptons) == 2:
                ...
                if len(tightElectrons) == 0 and abs(tightMuons_pdgid[0]+tightMuons_pdgid[1]) == 26:
                    ...
                    if self.is_mc:
                        if event.Muon_genPartFlav[ttc_l1_id] == 1 or event.Muon_genPartFlav[ttc_l1_id] == 15:
                            ttc_l1_isprompt = 1
                        if event.Muon_genPartFlav[ttc_l2_id] == 1 or event.Muon_genPartFlav[ttc_l2_id] == 15:
                            ttc_l2_isprompt = 1
                if len(tightElectrons) == 1 and abs(tightMuons_pdgid[0]+tightElectrons_pdgid[0]) == 24:
                    ...
                    if self.is_mc:
                        if event.Muon_genPartFlav[ttc_l1_id] == 1 or event.Muon_genPartFlav[ttc_l1_id] == 15:
                            ttc_l1_isprompt = 1
                        if event.Electron_genPartFlav[ttc_l2_id] == 1 or event.Electron_genPartFlav[ttc_l2_id] == 15:
                            ttc_l2_isprompt = 1
                if len(tightElectrons) == 2 and abs(tightElectrons_pdgid[0]+tightElectrons_pdgid[1]) == 22:
                    ...
                    if self.is_mc:
                        if event.Electron_genPartFlav[ttc_l1_id] == 1 or event.Electron_genPartFlav[ttc_l1_id] == 15:
                            ttc_l1_isprompt = 1
                        if event.Electron_genPartFlav[ttc_l2_id] == 1 or event.Electron_genPartFlav[ttc_l2_id] == 15:
                            ttc_l2_isprompt = 1
            ...
            if len(tightLeptons) == 1:
                ...
                if len(tightElectrons) == 1:
                    ...
                    if len(fakeable_Electrons) == 1 and abs(tightElectrons_pdgid[0]+fakeable_Electrons_pdgid[0]) == 22:
                        ...
                        ttc_drll = fakeable_Electrons[0].DeltaR(
                            tightElectrons[0])
                        ttc_dphill = fakeable_Electrons[0].DeltaPhi(
                            tightElectrons[0])
        ...
        if ttc_region > 0:
            ...
            l1_v4_temp.SetPtEtaPhiM(
                ttc_l1_pt, ttc_l1_eta, ttc_l1_phi, ttc_l1_mass)
            l2_v4_temp.SetPtEtaPhiM(
                ttc_l2_pt, ttc_l2_eta, ttc_l2_phi, ttc_l2_mass)
        ...
        if OPS_nl:
            if len(tightLeptons) == 2:
                ...
                if len(tightElectrons) == 0 and abs(tightMuons_pdgid[0]+tightMuons_pdgid[1]) == 0:
                    ...
                    if self.is_mc:
                        if event.Muon_genPartFlav[OPS_l1_id] == 1 or event.Muon_genPartFlav[OPS_l1_id] == 15:
                            OPS_l1_isprompt = 1
                        if event.Muon_genPartFlav[OPS_l2_id] == 1 or event.Muon_genPartFlav[OPS_l2_id] == 15:
                            OPS_l2_isprompt = 1
                # 2 eles case
                if len(tightElectrons) == 2 and abs(tightElectrons_pdgid[0]+tightElectrons_pdgid[1]) == 0:
                    ...
                    OPS_drll = tightLeptons[0].DeltaR(tightLeptons[1])
                    if self.is_mc:
                        if event.Electron_genPartFlav[OPS_l1_id] == 1 or event.Electron_genPartFlav[OPS_l1_id] == 15:
                            OPS_l1_isprompt = 1
                        if event.Electron_genPartFlav[OPS_l2_id] == 1 or event.Electron_genPartFlav[OPS_l2_id] == 15:
                            OPS_l2_isprompt = 1
                # 1 ele case
