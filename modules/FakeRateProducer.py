...
#    self.out.branch("nHad_tau", "I")
        ...
        self.has_EleHLT1 = bool(inputTree.GetBranch(
            "HLT_Ele8_CaloIdL_TrackIdL_IsoVL_PFJet30"))
        self.has_EleHLT2 = bool(inputTree.GetBranch(
            "HLT_Ele8_CaloIdM_TrackIdM_PFJet30"))
        self.has_EleHLT3 = bool(inputTree.GetBranch(
            "HLT_Ele12_CaloIdL_TrackIdL_IsoVL_PFJet30"))
        self.has_EleHLT4 = bool(inputTree.GetBranch(
            "HLT_Ele15_CaloIdL_TrackIdL_IsoVL_PFJet30"))
        self.has_EleHLT5 = bool(inputTree.GetBranch(
            "HLT_Ele17_CaloIdM_TrackIdM_PFJet30"))
        self.has_EleHLT6 = bool(inputTree.GetBranch(
            "HLT_Ele23_CaloIdL_TrackIdL_IsoVL_PFJet30"))
        self.has_EleHLT7 = bool(inputTree.GetBranch(
    ...
    def analyze(self, event):
        ...
        for imu in range(0, event.nMuon):
            ...
            muon_v4_temp.SetPtEtaPhiM(
                event.Muon_corrected_pt[imu], muons[imu].eta, muons[imu].phi, muons[imu].mass)
            ...
            if dr_mu_jet < 0.4:
                muon_conePt.append(
                    0.85*event.Jet_pt_nom[muon_closest_jetid_temp])
                muon_jet_Ptratio.append(
                    event.Muon_corrected_pt[imu]/(0.85*event.Jet_pt_nom[muon_closest_jetid_temp]))
                muon_closest_jetid.append(muon_closest_jetid_temp)
            else:
                muon_conePt.append(
                    event.Muon_corrected_pt[imu]*(1+event.Muon_miniPFRelIso_all[imu]))
                muon_jet_Ptratio.append(
        ...
        for imu in range(0, event.nMuon):
            if not self.is_mc:
                muon_genmatch.append(0)
            else:
                if (muons[imu].genPartFlav == 1 or muons[imu].genPartFlav == 15):
                    muon_genmatch.append(1)
                else:
                    muon_genmatch.append(0)
            # following cuts are preseletion for MVA muon ID
            if abs(muons[imu].eta) > 2.4 or muons[imu].sip3d > 8 or abs(muons[imu].dxy) > 0.05 or abs(muons[imu].dz) > 0.1 or muons[imu].miniPFRelIso_all > 0.4:
                continue
            if not (muons[imu].looseId and event.Muon_corrected_pt[imu] > 10 and muon_conePt[imu] > 10):
                continue
            if muons[imu].mediumId and muons[imu].tightCharge == 2:
                if muons[imu].mvaTTH > -0.2:
                    if (muon_conePt[imu] > 20):
                        muon_v4_temp.SetPtEtaPhiM(
                            muon_conePt[imu], muons[imu].eta, muons[imu].phi, muons[imu].mass)
                    ...
                    if muon_jet_Ptratio[imu] > 0.5 and event.Jet_btagDeepFlavB[muon_closest_jetid[imu]] < flav_cut(muon_conePt[imu], self.year):
                        if (muon_conePt[imu] > 20):
                            ...
                            if event.Muon_corrected_pt[imu] > 10:
                                muon_v4_temp.SetPtEtaPhiM(
                                    event.Muon_corrected_pt[imu], muons[imu].eta, muons[imu].phi, muons[imu].mass)
                                additional_looseMuons.append(
                                    muon_v4_temp.Clone())
                                additional_looseMuons_pdgid.append(
                                    muons[imu].pdgId)
                                additional_looseMuons_id.append(imu)
                    else:
                        if event.Muon_corrected_pt[imu] > 10:
        ...
        fakeable_Muons_id.extend(
            np.zeros(event.nMuon-len(fakeable_Muons_id), int)-1)
        ...
        for iele in range(0, event.nElectron):
            ...
            if dr_ele_jet < 0.4:
                electron_conePt.append(
                    0.85*event.Jet_pt_nom[electron_closest_jetid_temp])
                ...
                electron_conePt.append(
                    electrons[iele].pt*(1+event.Electron_miniPFRelIso_all[iele]))
                electron_jet_Ptratio.append(
        ...
        for iele in range(0, event.nElectron):
            if not self.is_mc:
                ele_genmatch.append(0)
            else:
                if (electrons[iele].genPartFlav == 1 or electrons[iele].genPartFlav == 15):
                    ele_genmatch.append(1)
                else:
                    ele_genmatch.append(0)
            # following cuts are preseletion for MVA electron ID
            if abs(electrons[iele].eta) > 2.5 or electrons[iele].sip3d > 8 or abs(electrons[iele].dxy) > 0.05 or abs(electrons[iele].dz) > 0.1 or electrons[iele].miniPFRelIso_all > 0.4:
                continue
            if not (electrons[iele].mvaFall17V2noIso_WPL and electrons[iele].lostHits < 2 and electrons[iele].pt > 10 and electron_conePt[iele] > 10 and electrons[iele].convVeto):
                continue
            if ((abs(electrons[iele].deltaEtaSC+electrons[iele].eta) < 1.479 and electrons[iele].sieie < 0.011) or (abs(electrons[iele].deltaEtaSC+electrons[iele].eta) > 1.479 and electrons[iele].sieie < 0.03)) and electrons[iele].hoe < 0.1 and electrons[iele].eInvMinusPInv > -0.04:
                if electrons[iele].mvaTTH > 0.25 and electrons[iele].tightCharge == 2:
                    if (electron_conePt[iele] > 20):
                        electron_v4_temp.SetPtEtaPhiM(
                            electron_conePt[iele], electrons[iele].eta, electrons[iele].phi, electrons[iele].mass)
                    ...
                    if (electron_conePt[iele] < 20 and electrons[iele].pt > 10):
                        electron_v4_temp.SetPtEtaPhiM(
                            electrons[iele].pt, electrons[iele].eta, electrons[iele].phi, electrons[iele].mass)
                        additional_vetoElectrons.append(
                    ...
                    if electrons[iele].mvaFall17V2noIso_WP90 and electron_jet_Ptratio[iele] > 0.6 and event.Jet_btagDeepFlavB[electron_closest_jetid[iele]] < flav_cut(electron_conePt[iele], self.year):
                        if (electron_conePt[iele] > 20):
                            ...
                            fakeable_Electrons_pdgid.append(
                                electrons[iele].pdgId)
        ...
        fakeable_Electrons_id.extend(
            np.zeros(event.nElectron-len(fakeable_Electrons_id), int)-1)
...
#    self.out.fillBranch("nHad_tau", nHad_tau)
        ...
        if not n_tight_muon+n_fakeable_muon + n_tight_ele+n_fakeable_ele == 1:
            return False
        if not n_loose_muon + n_loose_ele == 0:
            return False
        if n_tight_muon == 1:
            lep_genmatch = muon_genmatch[tightMuons_id[0]]
        if n_fakeable_muon == 1:
            lep_genmatch = muon_genmatch[fakeable_Muons_id[0]]
        if n_tight_ele == 1:
            lep_genmatch = ele_genmatch[tightElectrons_id[0]]
        if n_fakeable_ele == 1:
            lep_genmatch = ele_genmatch[fakeable_Electrons_id[0]]
        ...
        if (n_tight_muon+n_fakeable_muon) == 1:
            ...
            if not (MuHLT1 or MuHLT2 or MuHLT3 or MuHLT4 or MuHLT5 or MuHLT6 or MuHLT7 or MuHLT8 or MuHLT9 or MuHLT10):
                return False

        if (n_tight_ele+n_fakeable_ele) == 1:
            if self.has_EleHLT1:
                if event.HLT_Ele8_CaloIdL_TrackIdL_IsoVL_PFJet30:
                    EleHLT1 = True
            if self.has_EleHLT2:
                if event.HLT_Ele8_CaloIdM_TrackIdM_PFJet30:
                    EleHLT2 = True
            if self.has_EleHLT3:
                if event.HLT_Ele12_CaloIdL_TrackIdL_IsoVL_PFJet30:
                    EleHLT3 = True
            if self.has_EleHLT4:
                if event.HLT_Ele15_CaloIdL_TrackIdL_IsoVL_PFJet30:
                    EleHLT4 = True
            if self.has_EleHLT5:
                if event.HLT_Ele17_CaloIdM_TrackIdM_PFJet30:
                    EleHLT5 = True
            if self.has_EleHLT6:
                if event.HLT_Ele23_CaloIdL_TrackIdL_IsoVL_PFJet30:
                    EleHLT6 = True
            if self.has_EleHLT7:
                if event.HLT_Ele23_CaloIdM_TrackIdM_PFJet30:
                    EleHLT7 = True
            if not (EleHLT1 or EleHLT2 or EleHLT3 or EleHLT4 or EleHLT5 or EleHLT6 or EleHLT7):
                return False
...
#    if mt>40: return False
        ...
        for ijet in range(0, event.nJet):
            ...
            if jet_v4_temp.DeltaR(Leptons[0]) > 0.4:
                if event.Jet_pt_nom[ijet] > 35:
                    njet_35 = njet_35+1
                if event.Jet_pt_nom[ijet] > 30:
                    njet_30 = njet_30+1
                if event.Jet_pt_nom[ijet] > 25:
                    njet_25 = njet_25+1
                if event.Jet_pt_nom[ijet] > 20:
                    njet_20 = njet_20+1
                if abs(jets[ijet].eta) < 2.4 and jets[ijet].btagDeepB > 0.0532:
                    n_looseB = n_looseB+1
            if jet_v4_temp.DeltaR(Leptons[0]) > 0.7:
                if event.Jet_pt_nom[ijet] > 35:
                    njet_35_dr07 = njet_35_dr07+1
                if event.Jet_pt_nom[ijet] > 30:
                    njet_30_dr07 = njet_30_dr07+1
                if event.Jet_pt_nom[ijet] > 25:
                    njet_25_dr07 = njet_25_dr07+1
                if event.Jet_pt_nom[ijet] > 20:
                    njet_20_dr07 = njet_20_dr07+1
                if abs(jets[ijet].eta) < 2.4 and jets[ijet].btagDeepB > 0.0532:
                    n_looseB_dr07 = n_looseB_dr07+1

        if njet_35 > 0:
            jet_selection_35 = True
        if njet_30 > 0:
            jet_selection_30 = True
        if njet_25 > 0:
            jet_selection_25 = True
        if njet_20 > 0:
            jet_selection_20 = True
        if njet_35_dr07 > 0:
            jet_selection_35_dr07 = True
        if njet_30_dr07 > 0:
            jet_selection_30_dr07 = True
        if njet_25_dr07 > 0:
            jet_selection_25_dr07 = True
        if njet_20_dr07 > 0:
            jet_selection_20_dr07 = True
...
def FakeRate2016apv(): return FakeRateProducer("2016apv")
def FakeRate2016(): return FakeRateProducer("2016")
def FakeRate2017(): return FakeRateProducer("2017")
def FakeRate2018(): return FakeRateProducer("2018")
