...
class TopLeptonMvaModule(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        self.out.branch('Electron_topMVA_ID', 'I', lenVar='nElectron')
        self.out.branch('Muon_topMVA_ID', 'I', lenVar='nMuon')
    ...
    def analyze(self, event):
        ...
        lepton_WP_binning = np.array([0.0, 0.20, 0.41, 0.64, 0.81, 1.0])
        electron_topMVA_ID = np.digitize(electron_scores, lepton_WP_binning) - 1
        muon_topMVA_ID = np.digitize(muon_scores, lepton_WP_binning) - 1
 
        # fill branches
        self.out.fillBranch(self.electronvarname, electron_scores)
        self.out.fillBranch(self.muonvarname, muon_scores)
        self.out.fillBranch('Electron_topMVA_ID', electron_topMVA_ID)
        self.out.fillBranch('Muon_topMVA_ID', muon_topMVA_ID)
        return True

TopMVA2016apvProducer = lambda: TopLeptonMvaModule("2016PreVFP", "ULv1")
TopMVA2016postapvProducer = lambda: TopLeptonMvaModule("2016PostVFP", "ULv1")
TopMVA2017Producer = lambda: TopLeptonMvaModule("2017", "ULv1")
TopMVA2018Producer = lambda: TopLeptonMvaModule("2018", "ULv1")
