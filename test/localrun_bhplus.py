...
def main():
  ...
  if opt.ismc:
    if opt.year == "2016a":
      p = PostProcessor(opt.output, [opt.inputs], modules=[countHistogramsModule(),puAutoWeight_2016_preAPV(),PrefCorr2016(), mu_idisosf_2016APV(),muonScaleRes2016a(),ele_recoidsf_2016APV(), jmeCorrections_UL2016APVMC(), btagSF2016ULapv(), BH2016()], provenance=True,fwkJobReport=True, jsonInput=runsAndLumis(), outputbranchsel="keep_and_drop.txt",maxEntries=opt.nEvent)
    if opt.year == "2016b":
    ...
    if opt.year == "2017":
      p = PostProcessor(opt.output, [opt.inputs], modules=[countHistogramsModule(),puWeight_2017(),PrefCorr(),mu_idisosf_2017(),muonScaleRes2017(),ele_recoidsf_2017(), jmeCorrections_UL2017MC(),btagSF2017UL(), BH2017()], provenance=True,fwkJobReport=True, jsonInput=runsAndLumis(),outputbranchsel="keep_and_drop.txt",maxEntries=opt.nEvent)
    if opt.year == "2018":
      p = PostProcessor(opt.output, [opt.inputs], modules=[countHistogramsModule(),puAutoWeight_2018(),mu_idisosf_2018(),muonScaleRes2018(),ele_recoidsf_2018(), jmeCorrections_UL2018MC(), btagSF2018UL(), BH2018()], provenance=True,fwkJobReport=True, jsonInput=runsAndLumis(), outputbranchsel="keep_and_drop.txt",maxEntries=opt.nEvent)
